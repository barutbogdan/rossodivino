 ...
 * @method Field\SlimImage      slimImage($column, $label = '')
 ...
 * @method Field\HasManyTranslated hasManyTranslated($relationName, $callback)
...
class Form
{
    ...
    protected function prepare($data = [])
    {
        ...
        $this->inputs = $this->removeHiddenFields($this->inputs);

        $this->complete($this->preparing);
    ...
    /**
     * Remove hidden fields from input.
     *
     * @param array $input
     *
     * @return array
     */
    protected function removeHiddenFields($input)
    {
        array_forget($input, $this->hidden);
    ...
    /**
     * Set preparing callback.
     *
     * @param Closure $callback
     *
     * @return void
     */
    public function preparing(Closure $callback)
    {
    ...
    {
        $this->hidden = array_merge($this->hidden, (array) $fields);
    ...
    public function resource($slice = -2)
    {
        ...
        foreach ($params as $param => $value) {
            if ($value instanceof Model) {
                $path = str_replace('{' . $param . '}', $value->id, $path);
            } else {
