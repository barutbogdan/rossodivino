...
if (!function_exists('clear_optimized_images')) {
     ...
     * @return array
     */
    function clear_optimized_images($folder, $width = 100, $height = 100, $quality = 65, $path = 'upload')
    {
        $path = public_path(implode('/', [
            trim($path, '/'),
            trim($folder, '/'),
            $width,
            $height
        ...
        $images = array_filter((array) glob( $path . '/*'));
...
if (!function_exists('optimized_image')) {
     ...
     * @return string
     */
    function optimized_image($file, $width = 100, $height = 100, $quality = 65, $path = 'upload')
    {
        return app(\App\Packages\ImageOptimizer\ImageOptimizer::class)->optimize($file, $width, $height, $quality, $path);
    }
}
...
if (!function_exists('reduce_model_categories_to_css_classes')) {
    ...
    function reduce_model_categories_to_css_classes(Model $model)
    {
        ...
        return $model->categories->reduce(function ($carry, $category) {
            return $carry . ' ' . str_slug($category->getTranslation()->name);
        });
    }
}
...
if (!function_exists('page')) {
    ...
    function page(string $controller = '')
    {
        ...
        return array_get($pages, $controller, null);
    }
}
